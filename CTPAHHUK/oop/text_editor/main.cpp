...
void TestOneLine() {
    ...
    assert(editor.GetText() == "Hello, world!");
    assert(editor.GetCursorPosition() == 0);
    assert(editor.GetCharUnderCursor() == 'H');

    ApplyMultiple(editor, CB(CommandBuilder::Type::MoveCursorRight).build(), 8);
    assert(editor.GetText() == "Hello, world!");
    assert(editor.GetCursorPosition() == 8);
    assert(editor.GetCharUnderCursor() == 'o');

    ApplyMultiple(editor, CB(CommandBuilder::Type::MoveCursorRight).build(), 22);
    assert(editor.GetText() == "Hello, world!");
    assert(editor.GetCursorPosition() == 13);
    assert(editor.GetCharUnderCursor() == 0);

    ApplyMultiple(editor, CB(CommandBuilder::Type::MoveCursorLeft).build(), 57);
    assert(editor.GetText() == "Hello, world!");
    assert(editor.GetCursorPosition() == 0);
    assert(editor.GetCharUnderCursor() == 'H');
}
...
void TestCopyPaste() {
    ...
    editor.ApplyCommand(CB(CommandBuilder::Type::MoveCursorLeft).build());
    editor.ApplyCommand(CB(CommandBuilder::Type::MoveToStart).build());
    editor.ApplyCommand(CB(CommandBuilder::Type::SelectText).SelectionSize(5).build());
    editor.ApplyCommand(CB(CommandBuilder::Type::DeleteText).build());
    editor.ApplyCommand(CB(CommandBuilder::Type::DeleteText).build());
    assert(editor.GetCursorPosition() == 30);
    assert(editor.GetCharUnderCursor() == 'i');
    assert(editor.GetText() == "Fedor was here\nFedor was here\nis cool\n");
}
...
void TestUnselect() {
    ...
    editor.ApplyCommand(CB(CommandBuilder::Type::InsertText).Text("Who am I\nAyanami Rey\n").build());
    assert(editor.GetText() == "Who am I\nAyanami Rey\n");
    assert(editor.GetCursorPosition() == 21);
    assert(editor.GetCharUnderCursor() == 0);

    editor.ApplyCommand(CB(CommandBuilder::Type::MoveCursorUp).build());
    ApplyMultiple(editor, CB(CommandBuilder::Type::MoveCursorRight).build(), 4);
    assert(editor.GetText() == "Who am I\nAyanami Rey\n");
    assert(editor.GetCursorPosition() == 13);
    assert(editor.GetCharUnderCursor() == 'a');

    CommandBuilder::Type CommandsToTest[] = {
            CommandBuilder::Type::MoveCursorLeft,
            CommandBuilder::Type::MoveCursorRight,
            CommandBuilder::Type::MoveCursorUp,
            CommandBuilder::Type::MoveCursorDown,
            CommandBuilder::Type::CopyText,
            CommandBuilder::Type::PasteText,
            CommandBuilder::Type::MoveToEnd,
            CommandBuilder::Type::MoveToStart,
            CommandBuilder::Type::DeleteWord,
            CommandBuilder::Type::InsertText,
    };

    for (const auto& command : CommandsToTest) {
        editor.ApplyCommand(CB(CommandBuilder::Type::SelectText).SelectionSize(3).build());
        editor.ApplyCommand(CB(command).Text("123").build());
        assert(!editor.HasSelection());
    }

    assert(editor.GetText() == "Who am I\n123y\n");
    assert(editor.GetCursorPosition() == 12);
    assert(editor.GetCharUnderCursor() == 'y');
}
